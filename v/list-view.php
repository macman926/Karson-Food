<?php
$page_title="Single Order View (List)/ {$r['CUSTOMER_NAME']}/{$r['ORDER_TYPE']}";
$editButton = "<button type=button id='editMode' class='btn btn-sm btn-info'>Edit Mode</button>";
$jqFileIncAry=[
	CNFG__SiteDocRoot."/js/jq-ajax.php",
];
if($rights){
	$calTblOut=str_replace("^PLACEH^",$editStudentCount,$calTblOut);
	$calTblOut=str_replace("^PLACEY^",$studentCountInput,$calTblOut);
	$editOut = $editButton;
	$inputOut = $studentCountInput;
	array_push($jqFileIncAry, CNFG__SiteDocRoot."/js/jq-edit.php");
}
else{
	$calTblOut=str_replace("^PLACEH^",'',$calTblOut);
	$calTblOut=str_replace("^PLACEY^",'',$calTblOut);
...
<body>
	...
	<div id="body_content" class='<?=(isset($ditch_top_container_cls)&&$ditch_top_container_cls===true?'':'container')?> mt-5 ml-5 col'>
            <h4>Single Order View (List): <?="{$r['CUSTOMER_NAME']}/{$r['ORDER_TYPE']}"?></h4>
				<?=$page_out['body']??''?>
